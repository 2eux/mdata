...
// ✅ FIX: urutan step yang benar sesuai workflow
$steps = [
    'Request Submitted',
    'MDM Business Unit',
    'Direct Manager',
...
// ✅ FIX: ganti is_forwarded = 1 dengan status = 'APPROVED'
...
// Cek apakah user ini sudah approve/reject di step ini
$sudah_general = true;
...
<div class="container">
    ...
    <div class="card-view">
        ...
        <div class="status-box">
            ...
            <div class="status-badge badge-pending"><?= $header['status'] ?></div>
        </div>
    </div>
...
<!-- ✅ FIX: Button area — bersih, tidak nested -->
<div class="btn-area">
<?php if(!$sudah_general): ?>
    <form action="/atri/action/proses_review.php" method="POST" id="formAction">
        ...
        <button type="button" onclick="submitApprove()" class="btn-approve">✓ Approve</button>

        <button type="button" onclick="showReject()" id="btnReject" class="btn-reject">✗ Reject</button>

        <button type="button" onclick="submitReject()" id="btnRejectConfirm"
            class="btn-reject" style="display:none">✗ Confirm Reject</button>
...
<?php else: ?>
    <?php if($header['status'] == 'REJECTED'): ?>
        <span style="color:red;font-weight:bold">✗ Request sudah di-Reject.</span>
    <?php elseif($header['status'] == 'COMPLETED'): ?>
        <span style="color:green;font-weight:bold">✓ Request sudah selesai.</span>
    <?php else: ?>
        <span style="color:green;font-weight:bold">✓ Request sudah diproses di step ini.</span>
    <?php endif; ?>
<?php endif; ?>
</div>
