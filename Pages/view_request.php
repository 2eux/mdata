...
elseif($approvalData){
    ...
    elseif($approvalData['status'] == 'PENDING'){
        $icon = 'pending';
    }
    else{
        $icon = 'inactive';
    ...
    // OVERRIDE FROM APPROVAL DATA
    $approvalData = $approvals[$step] ?? null;

    if($approvalData){
        if($approvalData['status'] == 'REJECTED'){
            $icon = 'rejected';
        } elseif($approvalData['status'] == 'APPROVED' || $approvalData['status'] == 'GENERAL_APPROVED'){
            $icon = 'done';
        }
    }
    ...
    if($approvalData && !empty($approvalData['approved_by'])){
        $userQ = mysqli_query($koneksi, "SELECT nama FROM users WHERE id = ".$approvalData['approved_by']);
    ...
    if($approvalData && !empty($approvalData['approved_at'])){
        $approvedAt = date('l, d M Y H:i:s', strtotime($approvalData['approved_at']));
    }
?>
    <div class="step-item">
        ...
        <div class="table-box-view">
            <table class="table-view">
                ...
                <tbody>
                    ...
                    <?php endwhile; else: ?>
                    <tr>
                        <td colspan="8" style="text-align:center;padding:20px;color:#999">No data available</td>
                    </tr>
                    <?php endif; ?>
                </tbody>
            </table>
